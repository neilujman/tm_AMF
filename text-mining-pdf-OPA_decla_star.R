...
# extraction des indices des textes qui ne comporte qu'une seule ligne d'achat, et au moins un astérisque
# indices rangés dans ind.declastar.sub
ind.declastar <- c()
...
for(i in ind.declastar){
    ...
    if(s == 1){
        ind.declastar.sub <- c(ind.declastar.sub, i)
    }
}

extractedTexts_declaration_star <- extractedTexts_declaration[ind.declastar.sub]
...
filesNames_declaration_star <-
    filesNames[ind.declastar.sub]
dropbox_links_declaration_star <-
    paste0(
        "https://www.dropbox.com/home/AMF/",
        today,
        "/OPA?preview=",
...
if(NROW(extractedTexts_declaration_star) > 0) {
	for (i in seq_along(extractedTexts_declaration_star)) {
		extrait <- extractedTexts_declaration_star[[i]]
	...
	today <- format(system_date, "%Y-%m-%d")
	today %<>% rep(., NROW(extractedTexts_declaration_star))
	
	result_declaration_star <- 
		cbind(
			today,
			societe,
			operateur,
	...
	AMF_declaration_star <-
	    createSheet(wb = AMF_workbook, sheetName = "Offres publiques d'acquisition")
	
	
	if(!file.exists("./extracted_texts/OPA_temp.xlsx")){
	    row <- createRow(AMF_declaration_star, 1)
	    ...
	    addDataFrame(
	        x = column_titles,
	        sheet = AMF_declaration_star,
	        row.names = FALSE,
	        col.names = FALSE
	    )
	    saveWorkbook(AMF_workbook,
	                 "./extracted_texts/OPA_temp.xlsx")
	}
	existing_data <-
	    read.xlsx(
	        "./extracted_texts/OPA_temp.xlsx",
	...
	new_data <- rbind(existing_data, result_declaration_star)
	temp <- new_data [1, ]
	temp_2 <-
	...
	setColumnWidth(AMF_declaration_star, colIndex = 1, colWidth = 15)
	setColumnWidth(AMF_declaration_star, colIndex = 2, colWidth = 22)
	setColumnWidth(AMF_declaration_star, colIndex = 3, colWidth = 22)
	setColumnWidth(AMF_declaration_star, colIndex = 4, colWidth = 22)
	setColumnWidth(AMF_declaration_star, colIndex = 5, colWidth = 22)
	setColumnWidth(AMF_declaration_star, colIndex = 6, colWidth = 22)
	setColumnWidth(AMF_declaration_star, colIndex = 7, colWidth = 22)
	setColumnWidth(AMF_declaration_star, colIndex = 8, colWidth = 53)
	setColumnWidth(AMF_declaration_star, colIndex = 9, colWidth = 65)
	...
	ligne.titre <- getRows(AMF_declaration_star, rowIndex = 1)
